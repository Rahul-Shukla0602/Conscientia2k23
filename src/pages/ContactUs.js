          ...
          {teams.map((item,i)=>{
            if(item.id !== 1){
              return(
                ...
                <span className="designation">{item.designation}</span>
                <span className='links'>
                  <a href={`https://www.linkedin.com/in/${item.linkedin}/`}><box-icon name='linkedin-square' type='logo' color='rgba(252,250,250,1)' /></a>
                  <a href={`malito:${item.email}`}><box-icon type='solid' name='envelope' color='rgba(252,250,250,1)'/></a>
                  </span>
                </div>
              </div>
              )
